...
model = createpde();
%% GEOMETRY
w=0.015; h = 0.001; a = 0.001; b = 0.00025; d = 0.013;
...
[grads, centroids] = computeTriangleGradients(model.Mesh.Nodes.', model.Mesh.Elements.', u); % dr (rAphi), dz (rAphi) 
r_cent = centroids(:,1) + seps;  
grads(:,1)=grads(:,1)./r_cent; % 1/r dr (rAphi)
grads(:,2)=grads(:,2)./r_cent; % 1/r dz (rAphi) = dz Aphi
B= [-grads(:,2), grads(:,1)]; % B_r, B_z
normB=sqrt(dot(B, B, 2));

figure
hold on
patch('Faces', model.Mesh.Elements.', 'Vertices', model.Mesh.Nodes.', 'CData', (normB), 'FaceColor','flat','EdgeColor', 'none'); 
pdegplot(model, 'EdgeLabels', 'off', 'FaceLabels', 'off') ;
axis equal
title("B [T]");
xlabel("x")
ylabel("y")
...
L = 2*pi*pp(1)*evaluate(F,pp(1),pp(2));
disp("Inductance:")
disp(L)
